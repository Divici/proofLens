...
describe("ReviewReport (react-pdf component)", () => {
  ...
  it("formats the completed-at timestamp as a stable, human-readable UTC string", () => {
    const review = makeReviewFixture({
    ...
    const text = collectText(tree).join(" ");
    // Raw ISO must NOT appear — we want the human form.
    expect(text).not.toContain("2026-04-29T12:00:00.000Z");
    // Human form: "Apr 29, 2026, 12:00 PM UTC" (en-US, hour12, UTC tz).
    expect(text).toMatch(/Apr 29, 2026/);
    expect(text).toMatch(/12:00\s*PM/i);
    expect(text).toMatch(/UTC/);
  });

  it("falls back to the raw input string when createdAt is unparseable", () => {
    // Defensive: if a malformed timestamp slips through the wire schema
    // we should still render the raw value rather than "Invalid Date".
    const review = makeReviewFixture({ createdAt: "not-a-date" });
    const tree = ReviewReport({
      review,
      thumbnailDataUrl: "data:image/jpeg;base64,AAAA",
      appVersion: "0.1.0",
  ...
  it.each<{ status: OverallStatus; colour: string; label: string }>([
    { status: "pass", colour: "#16a34a", label: "Pass" },
    {
      status: "pass-with-warnings",
      colour: "#d97706",
      label: "Pass with warnings",
    },
    { status: "fail", colour: "#dc2626", label: "Fail" },
    {
    ...
    {
      status: "request-better-image",
      colour: "#6b7280",
      label: "Request better image",
    },
  ])(
    "renders verdict pill for %s with the correct colour + label",
    ({ status, colour, label }) => {
      const review = makeReviewFixture({ overall: status });
      const tree = ReviewReport({
        review,
        thumbnailDataUrl: "data:image/jpeg;base64,AAAA",
        appVersion: "0.1.0",
      ...
      // Find a VIEW whose style array carries our backgroundColor — that
      // is the verdict pill. The component composes [styles.verdictPill,
      // { backgroundColor: VERDICT_COLOURS[overall] }].
      const pill = all.find((el) => {
        ...
        return style.some(
          (s) =>
            s &&
            typeof s === "object" &&
            (s as { backgroundColor?: string }).backgroundColor === colour,
        );
      });
      expect(pill).toBeDefined();

      // The label text must match VERDICT_LABEL[status].
      const text = collectText(tree).join(" ");
